...
//抽奖成功事件
case class LuckyEvent(userId: Long, luckyMoney: Int)

//抽奖失败事件
case class FailureEvent(userId: Long, reason: String)
...
case class Lottery(
  ...
  def update(luckyMoney: Int) = {
    // 更新状态
    copy(remainAmount = remainAmount - luckyMoney)
  }
}
...
object PersistenceTest extends App {
  ...
  println(s"the ActorSystem logLevel is ${system.settings.LogLevel}")
...
class LotteryRun(lotteryActor: ActorRef, lotteryCmd: LotteryCmd) extends Runnable { //抽奖请求
  ...
  def run: Unit = {
    for { fut <- lotteryActor ? lotteryCmd } yield fut match { //根据不同事件显示不同的抽奖结果
